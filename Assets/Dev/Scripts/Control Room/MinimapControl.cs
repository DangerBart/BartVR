using System.Collections; using System.Collections.Generic; using UnityEngine; using UnityEngine.UI;  public class MinimapControl : MonoBehaviour {      [SerializeField]     private GameObject defaultMarker;     [SerializeField]     private GameObject plane;     private GameObject minimap;      private float xScale;     private float yScale;      float minRangeX;
    ...
    float maxRangeY;      // Use this for initialization     void Start () {         minimap = this.transform.gameObject;
        ...
        if (notification.POI == 0)         {             SetIrrelevantNotificationLocation(notification);             CreateNewMarker(notification);         }         else         {             SetRelevantNotificationLocation(notification);             CreateNewMarker(notification);         }      }      private void SetIrrelevantNotificationLocation(Notification notification)
    {         float corX = Random.Range(minRangeX, maxRangeX);         float corY = Random.Range(minRangeY, maxRangeY);         notification.MinimapLocation = new Vector2(corX, corY);
        ...
        Vector2 POILocation = new Vector2(-1 * (notification.POILocation.x * xScale), (-1 * (notification.POILocation.z * yScale)) + defaultMarker.GetComponent<RectTransform>().rect.height);         notification.MinimapLocation = POILocation + Random.insideUnitCircle * 60;         Debug.Log("Relevant location: " + notification.MinimapLocation);     }      private void CreateNewMarker(Notification notification)
    {
